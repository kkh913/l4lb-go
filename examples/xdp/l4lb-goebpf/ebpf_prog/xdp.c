...
static INLINE struct dest_info *hash_get_dest(struct pkt_meta *pkt) {
	__u32 key;
...
static INLINE bool parse_udp(void *data, __u64 off, void *data_end, struct pkt_meta *pkt) {
	struct udphdr *udp;
...
static INLINE bool parse_tcp(void *data, __u64 off, void *data_end, struct pkt_meta *pkt) {
	struct tcphdr *tcp;
...
static INLINE void set_ethhdr(struct ethhdr *new_eth, const struct ethhdr *old_eth, const struct dest_info *tnl, __be16 h_proto) {
	memcpy(new_eth->h_source, old_eth->h_dest, sizeof(new_eth->h_source));
	memcpy(new_eth->h_dest, tnl->dmac, sizeof(new_eth->h_dest));
	new_eth->h_proto = h_proto;
}

static INLINE int process_packet(struct xdp_md *ctx, __u64 off) {
